...
namespace SqlServerWebAdmin
{
    public partial class EditColumn : System.Web.UI.Page
    {
        ...
        protected void UpdateButton_Click(object sender, System.EventArgs e)
        {
            ...
            if (!database.Tables.Contains(Request["table"]))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    // Show error and quit
                    ErrorUpdatingColumnLabel.Visible = true;
                    ErrorUpdatingColumnLabel.Text = "The following error occured while trying to apply the changes.<br>" + Server.HtmlEncode(Utility.MessageFormat(ex));
            ...
            else
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        table.Columns.Add(newColumnInfo);
                        table.Alter();
                        //columnInfo.Alter();
                    }
                    catch (Exception ex)
        ...
        private Column NewColumn(Microsoft.SqlServer.Management.Smo.Table table, Column originalColumn)
        {
            Column newcolumnInfo = new Column(table, originalColumn.Name);
            newcolumnInfo.Default = DefaultValueTextbox.Text;
            newcolumnInfo.DataType = originalColumn.DataType;


            newcolumnInfo.Identity = PrimaryKeyCheckbox.Checked;
            newcolumnInfo.DataType.MaximumLength = Convert.ToInt32(LengthTextbox.Text);
            newcolumnInfo.Nullable = AllowNullCheckbox.Checked;
            newcolumnInfo.DataType.NumericPrecision = Convert.ToInt32(PrecisionTextbox.Text);
            newcolumnInfo.DataType.NumericScale = Convert.ToInt32(ScaleTextbox.Text);
            newcolumnInfo.Identity = IdentityCheckBox.Checked;
            newcolumnInfo.IdentitySeed = Convert.ToInt32(IdentitySeedTextbox.Text);
            newcolumnInfo.IdentityIncrement = Convert.ToInt32(IdentityIncrementTextbox.Text);
